 ...
 * Rule: 1.5 days at start of year, 16.5 days earned over the year (0.0452 per day)
 * Maximum: 18 days by end of year
...
function calculateAccruedLeaveDays(date = new Date()) {
  ...
  // Calculation: days elapsed × 0.0452 (per day) + 1.5 (base)
  // 0.0452 = 16.5 days / 365 days (total accrual needed / days in year)
  // This gives 1.5 days at start, 18 days at end
  const daysElapsed = Math.floor((date - yearStart) / (1000 * 60 * 60 * 24));
  const baseDays = 1.5;
  const dailyRate = 0.0452; // 16.5 days to accrue over 365 days
  
  // Calculate accrued days, capped at 18 days
  const accruedDays = Math.min(baseDays + (daysElapsed * dailyRate), 18.0);
...
async function getLeaveBalanceSummary(employeeId, year = new Date().getFullYear()) {
  try {
    ...
    return {
      employeeId,
      year,
      totalAllocated: parseFloat(balance.total_days_allocated),
      accruedToDate: accruedToday,
      daysUsed: parseFloat(balance.days_used),
      daysRemaining: parseFloat(balance.days_remaining),
