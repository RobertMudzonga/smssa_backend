...
function mapRowToColumns(row, availableColumns) {
    ...
    for (const key of Object.keys(row)) {
        ...
        const synonyms = {
            ...
            projectmanager2: 'project_manager_id_2',
            manager2: 'project_manager_id_2',
            project_manager_2: 'project_manager_id_2',
            secondmanager: 'project_manager_id_2',
            stage: 'status',
...
// Map stage names to project status values
function mapStageToStatus(stage) {
    ...
    return stageMapping[normalized] || null;
}
...
function prepareTypedValue(val, dataType) {
    ...
    // Convert null/undefined/empty to consistent null
    let raw = val;
    ...
    const integerTypes = ['integer','bigint','smallint'];
    const numericTypes = ['integer','bigint','smallint','numeric','decimal','real','double precision','money'];
    
    if (numericTypes.includes(lowerType)) {
        if (raw === '') return null;
        
        // For integer columns, convert decimal strings to integers (truncate)
        if (integerTypes.includes(lowerType)) {
            const num = Number(raw);
            if (!isNaN(num)) {
                return Math.trunc(num); // Truncate decimals to integer
            }
        }
...
async function insertRows(table, rows) {
    ...
    try {
        ...
        for (const row of rows) {
            ...
            // For projects table, use ON CONFLICT to handle duplicates
            let q = `INSERT INTO ${table} (${colList}) VALUES (${placeholders})`;
            if (table === 'projects' && columns.includes('project_name')) {
                // If project_name already exists, update all other fields
                const updateCols = columns.filter(c => c !== 'project_name' && c !== 'project_id' && c !== 'created_at');
                if (updateCols.length > 0) {
                    const updateSet = updateCols.map(c => `${c} = EXCLUDED.${c}`).join(', ');
                    q += ` ON CONFLICT (project_name) DO UPDATE SET ${updateSet}`;
                } else {
                    q += ` ON CONFLICT (project_name) DO NOTHING`;
                }
            }
...
async function enrichProjectRow({ mapped, raw, tableCols, tableColTypes, client, projectContext }) {
    ...
    if (tableCols.includes('project_manager_id')) {
        ...
        if (needsLookup) {
            ...
            if (mgrVal) {
                ...
                if (mgrId) {
                    mapped.project_manager_id = mgrId;
                } else {
                    // If lookup fails, set to null to avoid inserting a string into a numeric column
                    console.warn(`Project manager "${mgrVal}" not found in employees; setting project_manager_id to null`);
                    mapped.project_manager_id = null;
                }
            } else {
                // No manager value found, ensure it's null
                mapped.project_manager_id = null;
            }
        }
    }

    // Resolve second project manager name/email -> project_manager_id_2
    if (tableCols.includes('project_manager_id_2')) {
        const currentVal = mapped.project_manager_id_2;
        const needsLookup = currentVal === undefined || currentVal === null || currentVal === '' || isNaN(Number(currentVal));
        if (needsLookup) {
            const mgrVal = findValueByNormalized(raw, ['project_manager_2', 'projectmanager2', 'manager2', 'second_manager', 'secondmanager']);
            if (mgrVal) {
                const mgrId = await resolveEmployeeIdByNameOrEmail(mgrVal, client, projectContext);
                if (mgrId) {
                    mapped.project_manager_id_2 = mgrId;
                } else {
                    // If lookup fails, set to null to avoid inserting a string into a numeric column
                    console.warn(`Project manager 2 "${mgrVal}" not found in employees; setting project_manager_id_2 to null`);
                    mapped.project_manager_id_2 = null;
                }
            } else {
                // No manager value found, ensure it's null
                mapped.project_manager_id_2 = null;
    ...
    // Map stage to status if status column exists and stage value was provided
    if (tableCols.includes('status')) {
        // Check if we have a stage value in the raw row
        const stageValue = findValueByNormalized(raw, ['stage', 'current_stage', 'project_stage']);
        if (stageValue && (!mapped.status || mapped.status === '')) {
            const mappedStatus = mapStageToStatus(stageValue);
            if (mappedStatus) {
                mapped.status = mappedStatus;
                console.log(`Mapped stage "${stageValue}" to status "${mappedStatus}"`);
            }
        }
    }
...
async function importFromBuffer(buffer, options = {}) {
    ...
    for (const s of sheets) {
        ...
        // Always start with raw array parsing to properly detect headers
        const rowsAll = xlsx.utils.sheet_to_json(ws, { header: 1, defval: null });
        if (rowsAll.length === 0) continue;

        // Find the header row: first row with at least one meaningful cell
        let headerIndex = -1;
        ...
        if (headerIndex === -1 || headerIndex >= rowsAll.length - 1) {
            // No valid header row found with data rows after it
            continue;
        }
        ...
        // Check if headerRow looks like actual headers (contains normalized/recognizable field names)
        const hasRecognizableHeaders = headerRow.some(cell => {
        ...
        if (hasRecognizableHeaders) {
            // Build objects using headerRow as keys (header-based mapping)
            ...
            // Headers don't look recognizable, use positional mapping
            for (const arr of dataRows) {
                const objProspect = {};
                const objProject = {};
